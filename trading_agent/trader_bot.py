...
# df = pd.read_csv(r"C:\Users\user\Desktop\Uni_work\year_3\Sem_2\DIA\project code\COMP-3071-TradingBot\On_Chain_Metrics\testing strategies\LSTM_File_.csv")
...
# --- 3) PREPROCESS DATA TO MATCH TRAINING EXACTLY ---
# Create target column
df['PriceUSD_target'] = df['PriceUSD'].shift(-1)

# Add derived features (keep only what's needed upfront)
df['TxTfrCnt_per_address'] = df['TxTfrCnt'] / df['AdrActCnt']
df['SplyCur_per_address'] = df['SplyCur'] / df['AdrActCnt']

# Drop rows with NaNs from shifting (only the last row)
...
for t in range(time_steps, len(prices)):
    ...
    # skip all steps prior to the official test window
