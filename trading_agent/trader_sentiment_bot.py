# # sentiment_aware_LSTM_BTC_option1.py
...
# SENTIMENT_FOLDER = r"C:\Users\user\Desktop\Uni_work\year_3\Sem_2\DIA\project code\sentiment_data\google_search_data\sentiment_finbert\bitcoin"

# # --- 1) LOAD RAW ON-CHAIN DATA (no slicing yet) ---
# df_raw = pd.read_csv(
...
# # if UTC-aware, this will drop tz info; if naive, it’s a no-op
...
#     def __init__(self, name, model, scaler_X, scaler_y,
#                  initial_cash=100, sentiment_k=0.6):
#         self.name         = name
#         self.model        = model
...
#         Xs = self.scaler_X.transform(X_window)
#         Xs = Xs.reshape(1, *Xs.shape)
#         y_pred_s = self.model.predict(Xs, verbose=0)
#         p = self.scaler_y.inverse_transform(y_pred_s)[0,0]
#         return p * (1 + np.random.normal(0, 0.01))
...
#         super().__init__(name, model, scaler_X, scaler_y,
#                          initial_cash, sentiment_k)
...
#     def __init__(self, name, model, scaler_X, scaler_y,
#                  initial_cash=100, short_window=7, long_window=30,
#                  sentiment_k=0.6):
#         super().__init__(name, model, scaler_X, scaler_y,
#                          initial_cash, sentiment_k)
...
#     SentimentAwareMovingAverageTrader( "MovingAverageTrader",model, scaler_X, scaler_y),
# ]

# # --- 7) RUN BACKTEST WITH SKIP BEFORE START_DATE ---
...
#     for agent in agents:
#         # compute p_hat & p_adj here for logging
#         p_hat = agent.predict_price(X_win)
#         p_adj = p_hat * (1 + agent.sentiment_k * senti_score)

#         # log everything
#         print( 
#             f"{agent.name} | {curr_date} | "
#             f"price_now={price_now:.4f} | "
#             f"p_hat={p_hat:.4f} | "
#             f"p_adj={p_adj:.4f} | "
#             f"sentiment={senti_score:.4f}"
#         )

#         # now execute the actual trade decision
...
# plt.title('Sentiment-Aware Agent Portfolios (Dec 1–31)')
...
# sentiment_aware_LSTM_BTC_corrected.py
...
# --- 4) DEFINE SENTIMENT-AWARE AGENT CLASSES ---
class LSTMTradingAgent:
    def __init__(self, name, model, scaler_X, scaler_y, initial_cash=100, sentiment_k=0.2):
        self.name = name
        self.model = model
        self.scaler_X = scaler_X
        self.scaler_y = scaler_y
...
class SentimentAwareMovingAverageTrader(LSTMTradingAgent):
    def __init__(self, name, model, scaler_X, scaler_y, initial_cash=100, short_window=7, long_window=30, sentiment_k=0.2):
        super().__init__(name, model, scaler_X, scaler_y, initial_cash, sentiment_k)
        self.short_window = short_window
        self.long_window = long_window
        self.prices_seen = []
...
# --- 5) LOAD MODEL & SCALERS ---
model = load_model(r"C:\Users\user\Desktop\Uni_work\year_3\Sem_2\DIA\project code\COMP-3071-TradingBot\trading_agent\model\Bit_Coin_Price_Prediction_LSTM_Model_4chain.keras")
scaler_X = joblib.load(r"C:\Users\user\Desktop\Uni_work\year_3\Sem_2\DIA\project code\COMP-3071-TradingBot\trading_agent\scalers\btc_scaler_X_4ChainMetrics.pkl")
scaler_y = joblib.load(r"C:\Users\user\Desktop\Uni_work\year_3\Sem_2\DIA\project code\COMP-3071-TradingBot\trading_agent\scalers\btc_scaler_y_4ChainMetrics.pkl")
